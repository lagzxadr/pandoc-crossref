...
let
  ...
  f = { mkDerivation, base, containers, data-accessor
      ...
      mkDerivation {
        ...
        license = "GPL";
	configureFlags= [ "--ghc-option=-j1" "--ghc-option=-pgml=./ld-wrapper" ];
	buildDepends = with pkgs; [ glibc glibc.static zlib.static gmp5.static patchelf ];
      };
  ...
  drv = with pkgs.haskell.lib; justStaticExecutables (dontCheck (variant (haskellPackages.callPackage f {})));
